...
import sys
#import tkMessageBox
import heapq  #IMPORT HEAP Milestone 3
#import DataBase #FOR DATABASE Milestone 4
...
########################## INSERT A HEAP #####################################   
        gridLetters = ['C','E','F','D','B','A',]   #Mix up alphabet letters. It rearranges letters all the way through F. and not G & H
        
        heapq.heapify(gridLetters)  # Use heapify to rearrange the letters
        
        heapq.heappush(gridLetters,'G')  #Add 'G' element. So letters can be in order
        heapq.heappush(gridLetters,'H')  #Add 'H' element. The last element in the list
        
#############################################################################         
        for i in range(s.BoardDimension):    #Works with heap. Uses the arranged letters to set up the Board Dimensions
...
############################Search Algorithm####################################
        def linear_search(values, search_for):
            ...
            while search_at < len(values) and search_res is False:  # Match the value with each data element
        ...
        #gridLetters = ['C','E','F','D','B','A',] 
        print(linear_search(gridLetters, 'A'))  #Returns True
        print(linear_search(gridLetters, 'H'))  #Returns False
################################################################################
    ...
    def DrawStandard(s):
        s.ColourButton('Gray',-1,-2,2,1)    #Button: Standard Setup 'White'
